...
async function showAdminDashboard() {
  ...
  document.getElementById("user-info-nav").innerHTML = `
    <strong><i class="fas fa-user-shield"></i> ${currentAdmin.username}</strong><br>
  ...
  document.getElementById("admin-info").innerHTML = `
    <strong><i class="fas fa-user-shield"></i> ${currentAdmin.username}</strong><br>
    <small>${getDepartmentName(currentAdmin.department)} | ${currentAdmin.role.toUpperCase()}</small>
  `
...
function displayAdminStats(stats) {
  ...
  if (currentAdmin.role === "hod" || currentAdmin.role === "dean") {
    ...
    container.innerHTML = `
      ...
      <div class="stat-card danger">
        ...
        <h4>New Responses</h4>
        <div class="stat-number">${Object.values(stats.departmentStats).reduce((sum, dept) => sum + dept.newResponses, 0)}</div>
      </div>
    `
  } else {
  ...
  const totalNewResponses =
    currentAdmin.role === "hod" || currentAdmin.role === "dean"
      ? Object.values(stats.departmentStats || {}).reduce((sum, dept) => sum + dept.newResponses, 0)
      : stats.newResponses
...
function displayClearanceStatus(clearance) {
  ...
  const approvedCount = Object.values(clearance.departments).filter((d) => d.status === "approved").length
  ...
  departments.forEach((dept) => {
    ...
    const statusBadge = `<span class="status-badge status-${deptData.status}">${deptData.status.toUpperCase()}</span>`
    let content = ""

    if (deptData.status === "approved") {
      content = `
                <p><strong>Approved by:</strong> ${deptData.approvedBy}</p>
                <p><strong>Date:</strong> ${new Date(deptData.approvedAt).toLocaleDateString()}</p>
            `
    ...
    modalBody.innerHTML = files
      ...
      <div class="file-item">
        ...
        <div class="file-actions">
          <button class="btn btn-primary" onclick="previewFile('${file.filename}', '${file.originalName}', '${file.mimetype}')">
            <i class="fas fa-eye"></i> Preview
          </button>
          <button class="btn btn-secondary" onclick="downloadFile('${file.filename}', '${file.originalName}')">
...
async function loadClearanceRequests(page = 1, status = "all", search = "") {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE}/api/admin/clearance-requests?${params}`, {
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
    })
...
function displayClearanceRequests(clearances) {
  ...
  clearances.forEach((clearance) => {
    ...
    const hasNewResponse = Object.values(clearance.departments).some((dept) => dept.hasNewResponse)
    if (hasNewResponse) {
    ...
    departmentsToShow.forEach((deptKey) => {
      ...
      const canManage =
        currentAdmin.department === deptKey || currentAdmin.role === "hod" || currentAdmin.role === "dean"

      // Determine button states
      const isProcessed = dept.status === "approved" || dept.status === "rejected"
      ...
      departmentContent += `
        <div class="department-section">
          ...
          <span class="status-badge status-${dept.status}">${dept.status.toUpperCase()}</span>
          ${dept.hasNewResponse ? '<span class="status-badge" style="background: var(--warning-color); color: white;">NEW RESPONSE</span>' : ""}
          ${dept.reason ? `<div class="rejection-reason"><strong>Reason:</strong> ${dept.reason}</div>` : ""}
          ${dept.approvedBy ? `<p><small><strong>Processed by:</strong> ${dept.approvedBy} on ${new Date(dept.approvedAt).toLocaleDateString()}</small></p>` : ""}
          
          ${
            canManage
              ? `
            <div class="action-buttons">
              <button class="btn btn-success ${approveClass}" ${approveDisabled} onclick="updateClearanceStatus('${clearance._id}', '${deptKey}', 'approved')">
                <i class="fas fa-check"></i> ${dept.status === "approved" ? "Approved" : "Approve"}
              </button>
              <button class="btn btn-danger ${rejectClass}" ${rejectDisabled} onclick="showRejectionForm('${clearance._id}', '${deptKey}')">
                <i class="fas fa-times"></i> ${dept.status === "rejected" ? "Rejected" : "Reject"}
              </button>
              <button class="btn btn-secondary" onclick="viewUploadedFiles('${clearance.studentId._id}', '${deptKey}')">
                <i class="fas fa-files"></i> View Files
              </button>
            </div>
            <div class="rejection-form" id="rejection-form-${clearance._id}-${deptKey}">
              <div class="form-group">
                <label><i class="fas fa-comment"></i> Reason for rejection:</label>
                <textarea id="rejection-reason-${clearance._id}-${deptKey}" rows="3" placeholder="Provide a clear reason for rejection..."></textarea>
              </div>
              <div class="action-buttons">
                <button class="btn btn-danger" onclick="rejectClearance('${clearance._id}', '${deptKey}')">
                  <i class="fas fa-times"></i> Confirm Rejection
                </button>
                <button class="btn btn-secondary" onclick="hideRejectionForm('${clearance._id}', '${deptKey}')">
    ...
    card.innerHTML = `
      <div class="request-header">
        <div class="student-info">
          <h4><i class="fas fa-user-graduate"></i> ${clearance.studentId.firstName} ${clearance.studentId.lastName}</h4>
          <p><i class="fas fa-id-card"></i> ${clearance.studentId.registrationNumber} | <i class="fas fa-envelope"></i> ${clearance.studentId.email}</p>
        </div>
        <div class="request-date">
          <small><i class="fas fa-calendar"></i> Requested: ${new Date(clearance.createdAt).toLocaleDateString()}</small>
        </div>
      </div>
      ${departmentContent}
    `
...
function displayPagination(pagination) {
  ...
  let paginationHTML = `
    <button class="pagination-btn" ${pagination.currentPage === 1 ? "disabled" : ""} onclick="loadClearanceRequests(${pagination.currentPage - 1}, '${currentStatus}', '${currentSearch}')">
  ...
  for (let i = startPage; i <= endPage; i++) {
    paginationHTML += `
      <button class="pagination-btn ${i === pagination.currentPage ? "active" : ""}" onclick="loadClearanceRequests(${i}, '${currentStatus}', '${currentSearch}')">
        ${i}
      </button>
    `
  }
  ...
  paginationHTML += `
    <button class="pagination-btn" ${pagination.currentPage === pagination.totalPages ? "disabled" : ""} onclick="loadClearanceRequests(${pagination.currentPage + 1}, '${currentStatus}', '${currentSearch}')">
  ...
  paginationHTML += `
    <div class="pagination-info">
      Showing ${(pagination.currentPage - 1) * pagination.itemsPerPage + 1} - ${Math.min(pagination.currentPage * pagination.itemsPerPage, pagination.totalItems)} of ${pagination.totalItems} requests
    </div>
  `
...
function displayClearedStudents(students) {
  ...
  if (students.length === 0) {
    container.innerHTML = "<p>No students have completed their clearance yet.</p>"
    return
  }
            ...
            ${students
              .map(
                ...
                <div class="request-card">
                    <div class="request-header">
                        <div class="student-info">
                            <h4>${student.studentId.firstName} ${student.studentId.lastName}</h4>
                            <p>${student.studentId.registrationNumber} | ${student.studentId.email}</p>
                        </div>
                        <div class="completion-date">
                            <span class="status-badge status-approved">CLEARED</span>
                            <small>Completed: ${new Date(student.updatedAt).toLocaleDateString()}</small>
                        </div>
                    </div>
                </div>
            `,
...
function exportClearedStudents() {
  ...
  fetch(`${API_BASE}/api/admin/cleared-students`, {
    ...
    .then((students) => {
      const csvContent = [
        ["Registration Number", "First Name", "Last Name", "Email", "Completion Date"],
      ...
      a.href = url
      a.download = `cleared_students_${new Date().toISOString().split("T")[0]}.csv`
      a.click()
...
async function updateClearanceStatus(clearanceId, department, status) {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/admin/clearance/${clearanceId}/${department}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${authToken}`,
    ...
    if (response.ok) {
      ...
      if (departmentSection) {
        ...
        const newResponseBadge = departmentSection.querySelector('[style*="background: var(--warning-color)"]')
        if (newResponseBadge) {
          newResponseBadge.remove()
        }

        // Add processed by information
        const existingProcessedInfo = departmentSection.querySelector("p small")
        if (existingProcessedInfo) {
          existingProcessedInfo.remove()
        }

        const processedInfo = document.createElement("p")
        processedInfo.innerHTML = `<small><strong>Processed by:</strong> ${currentAdmin.username} on ${new Date().toLocaleDateString()}</small>`

        // Insert after status badge
        const statusBadgeParent = statusBadge.parentNode
        statusBadgeParent.insertBefore(processedInfo, statusBadge.nextSibling)
      }
      ...
      if (approveBtn && rejectBtn) {
        ...
        if (status === "approved") {
          approveBtn.className = "btn btn-success completed"
...
function showRejectionForm(clearanceId, department) {
  const form = document.getElementById(`rejection-form-${clearanceId}-${department}`)
  if (form) {
...
async function rejectClearance(clearanceId, department) {
  const reasonTextarea = document.getElementById(`rejection-reason-${clearanceId}-${department}`)
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (departmentSection) {
        ...
        const existingReason = departmentSection.querySelector(".rejection-reason")
        if (existingReason) {
        ...
        const statusBadges = departmentSection.querySelectorAll(".status-badge")
        const lastBadge = statusBadges[statusBadges.length - 1]
        lastBadge.parentNode.insertBefore(rejectionReasonDiv, lastBadge.nextSibling)
        ...
        const processedInfo = document.createElement("p")
        processedInfo.innerHTML = `<small><strong>Processed by:</strong> ${currentAdmin.username} on ${new Date().toLocaleDateString()}</small>`
        rejectionReasonDiv.parentNode.insertBefore(processedInfo, rejectionReasonDiv.nextSibling)
      }
